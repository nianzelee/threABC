...
// NZ : use to write out the mapped netlist
#define THRESHOLD
...
//	Tim_ManPrint( pIfMan->pManTim );
    ...
    if ( pIfMan->pPars->fDelayOpt || pIfMan->pPars->fDsdBalance || pIfMan->pPars->fUserRecLib )
	 ...
	 // NZ : before pIfMan is stopped, constructed TH netlist!
#if 1
	 extern Vec_Ptr_t * cut2Th( If_Man_t * );
    extern Vec_Ptr_t * Th_CopyList( Vec_Ptr_t * );
	 if ( pIfMan->pPars->fUseDsdTune ) {
         cut_TList     = cut2Th( pIfMan );
         current_TList = Th_CopyList( cut_TList );
     }
#endif
    If_ManStop( pIfMan );
    // transfer name
